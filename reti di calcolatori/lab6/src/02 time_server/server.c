...
void connection_handler(int socket_desc) {
    ...
    recv_bytes=0;
    while(recv_bytes<sizeof("TIME")){
       
        ret=recv(socket_desc, recv_buf+recv_bytes,sizeof("TIME")-recv_bytes,0);
        if(ret==-1){
            if(errno==EINTR)
                continue;
            handle_error("recv");
    ...
    if(!strcmp("TIME",recv_buf)) handle_error("non è TIme");

    fprintf(stderr, "Message of %d bytes received\n", recv_bytes);
     ...
     */
     int bytes_send=0;
    while (bytes_send<server_message_len)
    {
        ret=send(socket_desc,send_buf+bytes_send,server_message_len-bytes_send,0);
        if(ret==-1){
            if(errno=EINTR)
                continue;
            handle_error("send");
